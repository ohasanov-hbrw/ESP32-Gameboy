...
static char *registersLookupTable[] = {
    "<NONE>",
    "A",
    "F",
    "B",
...
void instructionToString(cpuContext *CPU, char *str){
    instruction *inst = CPU->currentInstruction;
    ...
    switch(inst->mode) {
        ...
        case AM_R_A16:
            sprintf(str, "%s %s,$%04X", instructionName(inst->type), 
                registersLookupTable[inst->reg1], CPU->fetchedData);
            return;

        case AM_R:
            sprintf(str, "%s %s", instructionName(inst->type), 
                registersLookupTable[inst->reg1]);
            return;

        case AM_R_R: 
            sprintf(str, "%s %s,%s", instructionName(inst->type), 
                registersLookupTable[inst->reg1], registersLookupTable[inst->reg2]);
            return;
        ...
        case AM_R_A8:
            sprintf(str, "%s %s,$%02X", instructionName(inst->type), 
                registersLookupTable[inst->reg1], CPU->fetchedData & 0xFF);
            return;
        ...
        case AM_A8_R:
            sprintf(str, "%s $%02X,%s", instructionName(inst->type), 
                readBus(CPU->registers.pc - 1), registersLookupTable[inst->reg2]);
        ...
        case AM_A16_R:
            sprintf(str, "%s ($%04X),%s", instructionName(inst->type), 
                CPU->fetchedData, registersLookupTable[inst->reg2]);
            return;
